...
1.掌握Elasticsearch安装IK分词器安装方法

2.掌握Elasticsearch索引操作方法

3.掌握Elasticsearch文档操作训练

4.掌握Elasticsearch高级查询与DSL训练
...
在命令台定位到"D:\elasticsearch-8.15.0\bin"（elasticsearch安装目录下的bin文件）输入elasticsearch-plugin install https://get.infini.cloud/elasticsearch/analysis-ik/8.15.0回车即可运行安装。（我之前已经安装过所以报了一个already exists的错。）
...
**请求及结果（索引已经创建报错already exists, 正常情况会返回结果"erro:false".下同）**

![image-02](./images/image-02.png)



#### 	2.2.产品目录（Product Cata）索引
...
[^由于数据已经被我导入，以下结果通过先给出导入代码块再给出查询结果的方式呈现]: 



用Bulk API批量导入数据，需要将数据以NDJSON格式(每个JSON串之间不能换行)提交：
...
##### 5.查找注册日期在2023年1月1日到2023年12月31日之间的用户。
    ...
    "root_cause": [
      {
        ...
        "index_uuid": "XSBAurCWRmK827wFsVEAOg",
        "index": "orders"
      }
    ],
...
**1.**IK版本不匹配 报错IllegalArgumentException: Plugin [analysis-ik] was built for Elasticsearch version 8.12.2 but version 8.15.0 is running 命令中版本需匹配

**2.**在更新文档时用了PUT，导致除了我在下面提供的字段其他字段被删除：

![image-44](./images/image-44.png)

**解决方法**：如果在更新文档时只更新部分字段，要用POST操作而不是PUT。